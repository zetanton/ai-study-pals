<template>
  <div v-if="show" class="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
    <div class="bg-white rounded-2xl p-6 w-full max-w-md">
      <h2 class="text-2xl font-bold text-[#2e3856] mb-4">Add Child</h2>
      ...
      <div v-if="studentCode" class="mt-4 p-4 bg-green-50 rounded-lg">
        <p class="text-green-800 font-medium">Child added successfully!</p>
...
const { addChild, createChild } = useParentChildren()
...
const handleSubmit = async () => {
  try {
    ...
    if (mode.value === 'create') {
      ...
      await addChild(form.studentCode)
      form.studentCode = ''
    }
